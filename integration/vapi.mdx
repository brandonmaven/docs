...
Collect payments mid-call by transferring your Vapi customer to Maven's IVR, then transferring them back when payment is complete.

<Steps>
  <Step title="Create a Session">

  When your Vapi agent needs to collect a payment, create a Maven session from your server. In practice, this happens inside a Vapi server URL handler or custom function tool. The response includes an IVR `phone_number` to transfer the customer to.
  ...
  ivr_number = session["phone_number"]
  ```
  ...
  Use Vapi's `transferCall` tool to send the customer to the Maven IVR `phone_number` from Step 1. The customer will hear prompts and enter their card details using their phone keypad.
