...
Collect payments mid-call by transferring your Twilio customer to Maven's IVR, then transferring them back when payment is complete.

<Steps>
  <Step title="Create a Session">

  When your application needs to collect a payment, create a Maven session from your server. The response includes an IVR `phone_number` to transfer the customer to.
  ...
  Use the `phone_number` from the session response to transfer the customer to Maven's IVR. The customer will hear prompts and enter their card details using their phone keypad.
  ...
  dial = Dial()
  dial.number(ivr_number)  # phone_number from Step 1
