...
<ParamField body="channel" type="string" default="voice">
  Collection channel: `voice` or `sms`. Voice initiates an IVR call; SMS sends a checkout link.
</ParamField>

<ParamField body="input_mode" type="string" default="dtmf">
  How the customer enters card info on IVR: `dtmf` (keypad) or `asr` (speech recognition). Only applies to `voice` channel.
</ParamField>
...
<ResponseField name="checkout_url" type="string">
  Checkout URL for SMS channel sessions. `null` for voice sessions.
</ResponseField>
