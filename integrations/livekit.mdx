...
1. Set the `callback` parameter to your LiveKit SIP number when creating the session
2. Configure a dispatch rule in LiveKit to route returning calls to your agent
3. Maven will transfer the call back to your SIP number after payment completes

The webhook will notify your server of the payment result, which you can use to update your agent's context.

---

## Testing

Use a `mvn_test_` API key and test card `4242 4242 4242 4242`. See [Testing](/integrations/overview#testing) for details.
