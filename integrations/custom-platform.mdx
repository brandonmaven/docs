...
2. An app with a [payment gateway connected](/quickstart) (Stripe, Adyen, Checkout.com, Braintree, Authorize.net, Cybersource, Worldpay, Global Payments, J.P. Morgan, Fiserv, Shift4, NMI, Qualpay, Clover, Paysafe, USAePay, Moneris, CardConnect, Fortis, PayPal Payflow Pro, Opayo, Fat Zebra, Trust Payments, QuickBooks Payments, Ecommpay, or card forwarding)
...
| `gateway` | `stripe`, `authorizenet`, `braintree`, `shift4`, `fiserv`, `jpmorgan`, `nmi`, `qualpay`, `clover`, or `webhook` (card forwarding to your own endpoint) |
...
| `stripe_account` | [Stripe Platform Mode](/integrations/stripe-connect#platform-mode-multi-merchant-platforms) only — the connected account (`acct_...`) on your own Stripe platform to charge or vault on |
