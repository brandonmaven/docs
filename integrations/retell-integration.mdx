...
Maven integrates with [Retell AI](https://retellai.com) voice agents. When your agent needs to collect a payment, it calls the `collect_payment` custom function, Maven creates a session, and the caller is transferred to Maven's secure payment line for card collection.
...
5. Agent transfers the caller to Maven's secure payment line
...
4. Create an **API key** (Settings > API Keys) — starts with `mvn_test_` or `mvn_live_`
...
Maven matches incoming calls to sessions using the customer's phone number. When configuring the transfer in Retell, the call must preserve the **customer's real caller ID**.
...
4. When transferred to the payment line, use test card numbers:
