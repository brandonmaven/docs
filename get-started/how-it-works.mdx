...
<Steps>
  <Step title="Create a session">
    Your server calls `POST /v1/sessions` with the payment details. Maven returns a phone number.
  </Step>
  <Step title="Transfer the call">
    Your voice agent transfers the customer to the Maven phone number. The customer enters their card details using their phone keypad (DTMF).

    Maven matches the incoming call to the session using the caller's phone number. The transfer must preserve the original caller ID so it matches the `caller` value from Step 1.
  </Step>
  <Step title="Payment processes">
    Maven tokenizes the card and processes the charge on your connected Stripe account.
  </Step>
...
Sessions expire after **10 minutes** if the customer doesn't call the phone number. Creating a new session for the same phone number automatically cancels any previous active session.
