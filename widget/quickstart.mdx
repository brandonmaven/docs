...
<Steps>
  ...
  <Step title="Server — create a session">
    ...
    <Note>
    Using [Stripe Platform Mode](/integrations/stripe-connect#platform-mode-multi-merchant-platforms)?
    Add `"stripe_account": "acct_..."` to route the payment to one of your connected accounts —
    it's required when a platform key is configured.
    </Note>
